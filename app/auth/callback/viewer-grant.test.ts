...
vi.mock('@/lib/identity/composition',()=>({identityProvider:async()=>({getIdentity:async()=>({provider:'neon',subject:'fixture-subject',email:'fixture@example.test',verified:true})})}));
vi.mock('@/lib/identity/users',()=>({resolveApplicationUser:async()=>({id:'user',email:'fixture@example.test',verified:true})}));
vi.mock('@/lib/workspace/bind-workspace',()=>({bindWorkspaceToUser:async()=>({kind:'none'})}));
