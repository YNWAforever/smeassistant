...
export async function POST(
  ...
  try {
    const result = await decideVersion(auth.repository, {
      versionId,
    ...
    return json({
      ...
      idempotent: result.kind === "already-decided",
    ...
    console.error("[api/versions/request-changes] failed", {
      category: "version_decide_failed",
    });
