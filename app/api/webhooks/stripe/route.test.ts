...
const db = vi.hoisted(() => ({
  workspaces: new Map<
    string,
    { id: string; tier: string; stripe_customer_id: string | null }
  >(),
...
vi.mock("@/lib/repositories/billing", () => ({
  billingRepository: () => ({
    findWorkspace: async (id: string | null, customer: string | null) =>
      id
        ? (db.workspaces.get(id) ?? null)
        : ([...db.workspaces.values()].find(
            (row) => row.stripe_customer_id === customer,
          ) ?? null),
    applyTier: async (id: string, tier: string, stripeEventId: string) => {
      if (!db.tierEvents.some((row) => row.stripe_event_id === stripeEventId))
        db.tierEvents.push({
          workspace_id: id,
          tier,
...
function subscriptionEvent(
  ...
  return {
    id,
    type,
    data: { object: { id: `sub_${id}`, customer: customerId, status } },
  };
}
...
describe("POST /api/webhooks/stripe", () => {
  ...
  it("sets tier to paid on checkout.session.completed only after confirming current subscription status", async () => {
    db.workspaces.set(WS, {
      id: WS,
      tier: "lite",
      stripe_customer_id: "cus_abc",
    });
    constructEvent.mockReturnValue(
      checkoutCompletedEvent("evt_1", WS, "cus_abc"),
    );
    retrieveSubscription.mockResolvedValue(
      currentSubscription("sub_evt_1", "cus_abc", "active", WS),
    );
  ...
  it("does not grant paid access when Checkout completes with an incomplete subscription", async () => {
    db.workspaces.set(WS, {
      id: WS,
      tier: "lite",
      stripe_customer_id: "cus_inc",
    });
    constructEvent.mockReturnValue(
      checkoutCompletedEvent("evt_inc", WS, "cus_inc"),
    );
    retrieveSubscription.mockResolvedValue(
      currentSubscription("sub_evt_inc", "cus_inc", "incomplete", WS),
    );
  ...
  it("downgrades an unpaid subscription found by Stripe customer id", async () => {
    ...
    retrieveSubscription.mockResolvedValue(
      currentSubscription("sub_evt_unpaid", "cus_unpaid", "unpaid"),
    );
  ...
  it("keeps access during past_due without writing a redundant tier event", async () => {
    ...
    retrieveSubscription.mockResolvedValue(
      currentSubscription("sub_evt_due", "cus_due", "past_due"),
    );
  ...
  it("returns 500 for an unknown current subscription status instead of guessing entitlement", async () => {
    ...
    retrieveSubscription.mockResolvedValue(
      currentSubscription("sub_evt_x", "cus_x", "future_status"),
    );
  ...
  it("is idempotent -- a duplicate event id re-applies the tier but records one event", async () => {
    ...
    constructEvent.mockReturnValue(
      checkoutCompletedEvent("evt_3", WS, "cus_ghi"),
    );
    retrieveSubscription.mockResolvedValue(
      currentSubscription("sub_evt_3", "cus_ghi", "active", WS),
    );
  ...
  it("returns 500 (so Stripe retries) when the checkout references an unknown workspace", async () => {
    constructEvent.mockReturnValue(
      checkoutCompletedEvent("evt_5", WS, "cus_none"),
    );
    retrieveSubscription.mockResolvedValue(
      currentSubscription("sub_evt_5", "cus_none", "active", WS),
    );
    expect((await POST(webhookRequest())).status).toBe(500);
  });
});
