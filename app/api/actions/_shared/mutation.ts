...
export function localeFrom(
  ...
  const candidate =
    typeof body?.locale === "string"
      ? body.locale
      : (req.headers.get("x-sme-locale") ?? "");
  return isLocale(candidate) ? candidate : DEFAULT_LOCALE;
}

async function authorizeScope<Scope extends ActionScope>(
  req: Request,
  scope: Scope | null,
  limit: RateLimitScope,
): Promise<Mutation<Scope>> {
  if (!scope) return { ok: false, response: json({ error: "not_found" }, 404) };
  const auth = await authorizeWorkspaceRequest(
    { id: scope.workspaceId },
    { minRole: "manager", locationId: scope.locationId ?? undefined },
  );
  if (!auth.ok)
    return { ok: false, response: json({ error: auth.code }, auth.status) };
  const repository = artifactRepository();
  try {
    await resolveActionRunContext(
      repository,
      scope.actionId,
      auth.membership,
      auth.user.id,
  ...
  if (!decision.allowed)
    return {
      ok: false,
      response: rateLimitedResponse(decision.retryAfterSeconds),
    };
  return {
    ok: true,
    repository,
...
export async function authorizeActionMutation(
  req: Request,
  actionId: string,
  limit: RateLimitScope,
): Promise<Mutation<ActionScope>> {
  if (!UUID_RE.test(actionId))
    return { ok: false, response: json({ error: "actionId is invalid" }, 400) };
  let scope: ActionScope | null;
  try {
    scope = await loadActionScope(artifactRepository(), actionId);
  } catch {
...
export async function authorizeVersionMutation(
  req: Request,
  versionId: string,
  limit: RateLimitScope,
): Promise<Mutation<VersionScope>> {
  if (!UUID_RE.test(versionId))
    return {
      ok: false,
      response: json({ error: "versionId is invalid" }, 400),
    };
  let scope: VersionScope | null;
  try {
    scope = await loadVersionScope(artifactRepository(), versionId);
  } catch {
    return { ok: false, response: json({ error: "unavailable" }, 503) };
  }
  return authorizeScope(req, scope, limit);
...
export function objectiveDedupeKey(
  workspaceId: string,
  ...
  const hash = createHash("sha256")
    .update(objective.trim().toLowerCase())
    .digest("hex")
    .slice(0, 8);
