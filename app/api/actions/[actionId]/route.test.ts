...
vi.mock("@/lib/workspace/queries", () => ({
  loadWorkspaceContext: async () => ({ workspace: { id: WORKSPACE_ID } }),
}));
vi.mock("@/lib/workspace/queries-pages", () => ({
  getAction: async () => ({
    action: { id: ACTION_ID, actionState: "dismissed" },
  }),
}));

const PARAMS = { params: Promise.resolve({ actionId: ACTION_ID }) };
const patch = (body: unknown) =>
  import("./route").then(({ PATCH }) =>
    PATCH(
      new Request(`https://app.test/api/actions/${ACTION_ID}`, {
        method: "PATCH",
...
function respond(q: Query): unknown {
  ...
  return {
    ...
    required_inputs: ["brand_voice", "channel"],
...
describe("PATCH /api/actions/[actionId]", () => {
  it("dismisses an action and records action.dismissed", async () => {
    ...
    const update = mocks.db!.calls.find(
      (c) => c.table === "actions" && c.op === "update",
    );
    ...
    const audit = mocks.db!.calls.find((c) => c.table === "audit_events")
