...
vi.mock("@/lib/llm", () => ({
  llmComplete: (...args: unknown[]) => mocks.llmComplete(...args),
}));
...
const action = {
  ...
  title: { en: "Ask", "zh-HK": "邀請", "zh-TW": "邀請" },
  ...
  effort_minutes: 8,
  required_inputs: [],
  provided_inputs: { brand_voice: "warm", channel: "WhatsApp" },
...
function respond(q: Query): unknown {
  switch (q.table) {
    case "actions":
      return q.op === "select" ? actionRow : null;
    case "workspaces":
      return {
        business_name: "Shop",
        market: "hk",
    ...
    case "scan_snapshots":
      return [];
    case "action_runs":
      return q.op === "insert" ? { id: "run-1" } : null;
    default:
      return null;
  }
}
...
const post = (body: unknown = {}) =>
  POST(
    new Request(`https://app.test/api/actions/${ACTION_ID}/run`, {
      method: "POST",
...
beforeEach(() => {
  ...
  mocks.db.rpc.mockResolvedValue({
    data: { kind: "created", version_id: "v-1", version_no: 1 },
    error: null,
  });
  ...
  mocks.llmComplete.mockResolvedValue({
    text: JSON.stringify({
      title: "Ask",
      body: "Thanks for visiting — would you leave us a Google review?",
      acceptance_criteria: [],
      warnings: [],
      facts_used: [],
      facts_needed: [],
...
describe("POST /api/actions/[actionId]/run", () => {
  it("runs the agent and creates v1 for an owner", async () => {
    ...
    expect(mocks.llmComplete).toHaveBeenCalledWith(
      ...
      { jsonMode: true, temperature: 0.4, maxTokens: 1200, timeoutMs: 45_000 },
    );
    expect(mocks.db!.rpc).toHaveBeenCalledWith(
      "create_output_version",
      expect.objectContaining({
        p_author_type: "agent",
        p_action_run_id: "run-1",
      }),
    );
    expect(
      mocks
        .db!.calls.filter((c) => c.table === "audit_events")
        .map((c) => (c.payload as { event: string }).event),
    ).toEqual(["run.started", "run.succeeded"]);
  });
  ...
  it("403s a viewer and a manager outside the action's location scope", async () => {
    ...
    mocks.authorizeWorkspaceRequest.mockImplementation(
      authorizeLike("manager", ["some-other-location"]),
    );
    expect((await post()).status).toBe(403);
