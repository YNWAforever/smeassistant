...
export async function POST(
  ...
  const market =
    workspace.market === "hk" || workspace.market === "tw"
      ...
      : null;
  if (!market) {
    console.error("[owner/checkout-link] workspace has no valid market", {
      category: "checkout_link_market_missing",
    });
  ...
  if (!priceId) {
    return NextResponse.json(
      { error: `Stripe price for ${market} is not configured` },
      { status: 500 },
    );
  }
  ...
  try {
    ...
    if (!customerId) {
      ...
      try {
        await billingRepository().saveCustomer(workspaceId, customerId);
      } catch {
        console.error("Failed to persist stripe_customer_id", {
          category: "stripe_customer_save_failed",
        });
        return NextResponse.json(
