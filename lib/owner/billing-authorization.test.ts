...
vi.mock("@/lib/repositories/billing", () => ({
  billingRepository: () => ({ workspace: mocks.workspaceMaybeSingle }),
}));
...
describe("loadWorkspaceBillingContext", () => {
  ...
  it("asks for owner access -- billing is an owner setting (§3.9)", async () => {
    ...
    mocks.workspaceMaybeSingle.mockResolvedValue(null);
  ...
  it("retains the unavailable-workspace result when the billing read fails", async () => {
    mocks.authorizeWorkspaceRequest.mockResolvedValue(OWNER);
    mocks.workspaceMaybeSingle.mockRejectedValue(
