...
vi.mock("@/lib/repositories/billing", () => ({
  billingRepository: () => makeDb(),
}));

function makeDb() {
  return {
    tierEvents: async () => state.tierEvents,
    workspace: async () => state.workspace,
    usage: async (_id: string, period: string, allowance: number | null) => {
      if (!state.usage) {
        state.inserts.push({ workspace_id: _id, period, allowance });
        state.usage = { period, approved_deliveries: 0, allowance };
      }
      return state.usage;
    },
  };
...
describe("getBilling", () => {
  it("binds tier, usage, tier events, Stripe customer presence and the market price", async () => {
    ...
    expect(model.marketPrice).toEqual({
      amount: 888,
      currency: "HKD",
  ...
  it("reports a Stripe customer and the TW price for a tw workspace", async () => {
    ...
    const model = await getBilling({
      ...ctx,
      workspace: { ...ctx.workspace, market: "tw", tier: "paid" },
    });
...
describe("readUsage", () => {
  it("returns the existing row without inserting", async () => {
    ...
    const usage = await readUsage(makeDb() as never, {
      workspaceId: "ws-1",
      tier: "paid",
      timezone: "Asia/Hong_Kong",
  ...
  it("creates the period row lazily with the tier allowance (lite → 3)", async () => {
    ...
    expect(state.inserts).toEqual([
      { workspace_id: "ws-1", period: "2026-09", allowance: 3 },
    ]);
    expect(usage).toEqual({
      period: "2026-09",
