...
const flow: AuthFlow = {
  locale: "en",
  claim: null,
  returnTo: "/en/owner/shops?tab=overview",
  method: "google",
};
const identity = {
...
describe("completeSignIn", () => {
  it("finishes a returning accepted member at a validated original destination", async () => {
    const subject = ports();
    await expect(completeSignIn(flow, subject)).resolves.toEqual({
      kind: "redirect",
      destination: "/en/owner/shops?tab=overview",
    });
  ...
  it("clears an absent session and returns invalid_session", async () => {
    ...
    await expect(completeSignIn(flow, subject)).resolves.toMatchObject({
      kind: "recover",
  ...
  it("clears an unverified mapped identity without exposing identity fields", async () => {
    const subject = ports({
      mapIdentity: vi.fn().mockResolvedValue({ ...user, verified: false }),
    });
  ...
  it.each([
    [
      "provider outage",
      { getIdentity: vi.fn().mockRejectedValue(new Error("provider down")) },
      "fresh_session",
    ],
    [
      "mapping error",
      { mapIdentity: vi.fn().mockRejectedValue(new Error("mapping down")) },
      "identity_mapping",
    ],
    [
      "binding error",
      { bindInvitations: vi.fn().mockRejectedValue(new Error("binding down")) },
      "invitation_binding",
    ],
    [
      "membership query failure",
      {
        hasAcceptedMembership: vi.fn().mockRejectedValue(new Error("db down")),
      },
      "workspace_lookup",
    ],
    ...
    expect(subject.reportFailure).toHaveBeenCalledWith(
      stage,
      expect.any(String),
    );
    expect(JSON.stringify(result)).not.toContain("owner@example.test");
  ...
  it("returns no_access only after an authoritative accepted-membership lookup", async () => {
    const subject = ports({
      hasAcceptedMembership: vi.fn().mockResolvedValue(false),
    });
    await expect(completeSignIn(flow, subject)).resolves.toEqual({
      kind: "no_access",
    });
  ...
  it("completes a validated claim before membership lookup", async () => {
    const subject = ports({
      finishClaim: vi
        .fn()
        .mockResolvedValue("/en/owner/onboarding?claim=abcdef"),
    });
    await expect(
      completeSignIn({ ...flow, claim: "abcdef" }, subject),
    ).resolves.toEqual({
      kind: "redirect",
      destination: "/en/owner/onboarding?claim=abcdef",
    });
  ...
  it("returns unavailable on a claim failure rather than no_access", async () => {
    ...
    await expect(
      completeSignIn({ ...flow, claim: "abcdef" }, subject),
    ).resolves.toMatchObject({ kind: "recover", reason: "unavailable" });
    expect(subject.reportFailure).toHaveBeenCalledWith(
      "claim_resolution",
      expect.any(String),
    );
  ...
  it.each(["/en/owner/%2e%2e/staff", "/en/owner/%252e%252e/staff"])(
    "rejects an encoded dot traversal returned by a claim port: %s",
    async (destination) => {
      const subject = ports({
        finishClaim: vi.fn().mockResolvedValue(destination),
      });
  ...
  it("allows an encoded local claim destination without dot traversal", async () => {
    const destination = "/en/owner/shops/%E9%A6%99%E6%B8%AF?tab=evidence";
    const subject = ports({
      finishClaim: vi.fn().mockResolvedValue(destination),
    });
