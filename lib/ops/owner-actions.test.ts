...
describe("ownerActionFor", () => {
  const cases: Array<[string, FailureItem, OwnerContext, string]> = [
    ...
    ["failed scan without a location, scoped manager", item("scan_failed", null), scopedManager, "contact_support"],
    ...
    ["failed draft without a location, scoped manager", item("draft_failed", null, "act-1"), scopedManager, "open_action"],
