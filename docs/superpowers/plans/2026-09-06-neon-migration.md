...
- [ ] Add accepted/unaccepted/revoked, cross-workspace, owner/manager/viewer and conflicting claim SQL cases. **Approved amendment — 2026-09-07:** pending workspace membership has no invitation-specific TTL, so do not claim or add expired-invitation SQL coverage. Fixture users come through app identity mapping, not writes to managed Auth tables.
- [ ] Port membership lookup and transactional invitation binding. Verified intended-recipient identity is mandatory before granting the invited role; require the membership to remain pending and unrevoked, then accept it transactionally. Managed sign-in-link expiry and replay rejection remain Auth/callback requirements from Task 6: an expired or replayed link is a no-op, while a later fresh link may accept a still-pending invitation. Sign-up alone grants no ownership; preserve current claim flags.
