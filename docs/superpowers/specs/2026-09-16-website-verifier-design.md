...
- **Duplicate `verified` rows are deduped, not made impossible.** `insert` carries a `NOT EXISTS` guard, which stops the ordinary case of two overlapping cron ticks appending the same evidence twice. It is not a unique index, so under READ COMMITTED two genuinely simultaneous inserts can still both land. Closing that needs a partial unique index in a new migration, which this slice did not take. The consequence is a duplicated evidence row, not a false claim: both rows say the same true thing, and the display reads the first.
- **The same-site check bounds the false-`verified` risk without eliminating it.** A fetch that lands on a different host than the stored `website_url` (ignoring a leading `www.`) yields `not_yet` and writes nothing, which covers parked domains and redirects to a seller's page. A soft-404 served in place, on the owner's own host, still presents a title and one `h1` and would satisfy an action whose only failing declared checks were those. Narrow, but it is the one remaining path that fails toward a claim rather than away from one.
