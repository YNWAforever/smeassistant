...
Invitations and merchant claims are separate from authentication. **Approved amendment — 2026-09-07:** a pending workspace invitation remains valid until it is accepted or explicitly revoked; it has no separate invitation TTL. Grant the invited role only after managed authentication verifies the intended recipient identity, the pending membership is still unaccepted and unrevoked, and the membership is accepted transactionally. Managed sign-in links retain provider-enforced expiry and replay protection; an expired or replayed link is a no-op and does not accept membership. A later fresh managed sign-in link for the same verified recipient may accept the still-pending invitation. Never assign ownership from an untrusted email field or Google login alone. Account linking must use verified provider identity and tested provider rules; matching unverified email is insufficient.
