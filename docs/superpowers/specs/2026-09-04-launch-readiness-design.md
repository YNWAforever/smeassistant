...
1. **Accessibility.** The public header call-to-action uses the existing teal button variant instead of white-on-lime (contrast 1.26 → passes 4.5). The brand link's `aria-label` becomes "SME Scanner · 主頁" / "SME Scanner · home" so it starts with the visible text. No other styling changes.
...
| 4 | Supabase → SQL editor | Apply `20260903000000_workspace_layer.sql` then `20260903000001_workspace_rpcs.sql` on a non-production project, then production | `launch:check --supabase` sees both tables. |
