...
| Discover CI scripts + runtime; test on the intended serving runtime | fixed this phase | `.nvmrc` moved 22 → **24** on Willy's decision, so CI's `setup-node` now installs the runtime production serves. `engines.node` stays a `>=22.13.0` floor, and the three stale `.nvmrc = 22` statements in `CLAUDE.md` plus the one in `docs/integration/DEPLOY.md` were corrected with it. Local evidence for 24.18.0 covers typecheck, lint and the full 2,320-test suite; **`build` and `e2e` have never run in this environment at all** (the pre-existing `radix-ui` module-resolution failure), so CI is the first place those two gates will meet Node 24. |
...
| Persist policy-versioned scan consent server-side; validate before dispatch | fixed this phase | `6da4d97` — reuses `consent_records` (`consent_type` is bare `text` with no CHECK and `lead_id` is nullable, so no migration; `verifyCatalog` deep-equals the whole catalog with no column allowlist, which rules one out anyway). `parseScanConsent` validates presence and exact policy-version equality before any database work and stamps the **server-resolved** version; `jobsRepository.insert` writes the job and its consent in one transaction, so a queued job cannot exist without consent; `assertScanConsent` re-reads it in `/api/scan/process` after the limiter and before any provider call, marking a consent-less job `failed` (`queued`-guarded, never racing the executor) so the existing failure card surfaces it. Rescan records a fresh row in the requester's UI locale and emits the previously-unused `consent.public_evidence` event. |
| Stop indefinite polling; bounded waiting/stalled/failed states and a safe resume | fixed this phase | `df8d36e` — a 15-minute / 50-attempt budget with tiered backoff (8 s cap until 90 s elapsed, then 30 s), and an explicit four-value view state driven by pure functions. The stalled card never claims failure: it says we stopped asking, and collectors still in flight show `stalled` with a **pending** badge, never `failed`. 404/400 are permanent, 429 stops rather than burning an exhausted bucket, five consecutive transport failures report unreachable; a healthy answer clears the stall and terminal always wins, so the card cannot contradict the progress bar. A per-job localStorage record keeps the honest elapsed clock separate from the budget, so a refresh resumes rather than buying fresh time. The resume note claims only what `claimJob` does (same scan, ~30 min, three attempts), pinned by a test that reads `execution-store.ts`. |
...
| Derive the starting step from persisted state, not React memory | fixed this phase | The resume step is now computed server-side from ownership plus whether `POST /api/workspaces/claim` has created the primary location — no query parameter is read, and the OAuth callback no longer emits the `claimed=1` it used to depend on, so an owner returning to the same URL is never dropped back to step 1 with their ownership already proven. Step 4's fields are seeded from `workspaces.business_name`, the primary `locations` row and `brand_profiles`, so a returning owner sees what was saved rather than the scan's guesses re-proposed over it; an unrecognised stored voice falls back to the default instead of becoming a dead Select value. Keystroke-level draft persistence for in-progress typing is deliberately **not** added — the step submits immediately, and the fields now reflect persisted state, which is what the requirement is about. |
| "Wrong business" escape and optional Instagram step | fixed this phase | Instagram was already genuinely skippable. Step 1 now carries the escape the pre-claim `/scan` flow had: while nothing is attached it is a plain link to `/{locale}/scan` — no mutation, no state change. Once the report **is** attached it deliberately becomes a support instruction rather than a button: ownership is proven, never self-declared (guardrail 15), so a self-service detach would be the same hijack primitive in reverse. Both branches are pinned by `components/onboarding-page.test.tsx`. |
...
| Requirement | State | Evidence |
|---|---|---|
| Reuse scan-sampled reviews; remove required manual re-entry | fixed this phase | `lib/workspace/evidence-inputs.ts` is one shared resolver: the selector moved out of `runs.ts` so the page, the derivation and the agent read the same bytes, and `resolveEvidenceInputs` subtracts `reviews_without_response` from `requiredInputs` before derivation persists them — so `missingInputs`, the readiness factor, the PATCH rule and the list card are all correct without a migration. The detail page resolves the same evidence live (mirroring `resolveActionRunContext`'s scope refusals, membership included) and renders the excerpts with an `Observed` fact type and an honest sample-bound note; the textarea is demoted to a labelled owner-supplied fallback. Deliberately narrow: `brand_voice`/`language` are read through `ctx.providedInputs`, so resolving them would blank the prompt and hide the input form. Fallback holds at three layers, and derivation's `ON CONFLICT` now returns an untouched action to `needs_input` when it loses its evidence. |
...
| Reap expired runs; prevent a late worker overwriting a newer attempt | fixed this phase | `813ac78` — reaps in application SQL with no migration (everything needed is already on `action_runs`, and `timed_out` is already legal in its CHECK). One statement does the transition and its audit row together: a `MATERIALIZED` locking CTE with `FOR UPDATE ... SKIP LOCKED`, so concurrent reapers take disjoint rows; `now()` is the database clock; the CTE repeats `ACTION_SCOPE_PREDICATE`'s locations check so the audit row can never carry a foreign location; it never touches `actions`. `RUN_STALE_AFTER_MS` is 3 minutes — 3× the platform's hard 60 s kill — with a test pinning it above 2×. No fencing token is needed: `loadRun`'s state precondition already makes a late worker's `finish()` throw. Recovery is owner-triggered discovery, not self-healing; the list page is deliberately not reaped and a test pins that. |
