...
The empty state now names the Fimmick team as the source, says plainly that a scan does not create them, and links Actions, where this workspace's own review-reply and GBP-post drafting genuinely is live. The heading changed too: "Replies and posts **drafted from scan findings**" carried the same implication that a scan produces them.
