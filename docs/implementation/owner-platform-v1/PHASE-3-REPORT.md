...
**Branch** `p35a-spend-budgets` · Task 10 candidate `90ef323` (the last code commit of the plan) and its record `6bdd438`; then four fixes from the whole-branch review, `5971d55`, `b600e77`, `838888e` and `a10a7d8`, and the documentation commit that records them (see "Whole-branch review and its fixes" at the end of this section) · Base: `main` at `96519fc` (PR #20, merged, P3.2c; `origin/main` still points there). Node `v24.18.0`, pnpm `9.12.0` via corepack, Windows 11, Docker Server `29.7.2`.

The sections from "What this closes" to "What this slice does NOT prove" describe `90ef323` unless they say otherwise; where the review changed a statement, the statement is corrected in place and names the fix commit.

Built from `docs/superpowers/plans/2026-09-25-spend-budgets.md` (Tasks 1–10), against the design in [`docs/superpowers/specs/2026-09-25-spend-budgets-design.md`](../../superpowers/specs/2026-09-25-spend-budgets-design.md), including its "Amendments made during planning (2026-09-25)".

**Implemented and locally verified. Nothing here is hosted-verified.** No deployment, no remote migration, no paid provider call, no push. Migration `0009` was applied only to disposable Docker Postgres: by `db:verify`, by the integration harness, and by the rollout rehearsal under "Runbook". The production statement `rollout/apply-0009.sql` has never been run against Neon.

### What this closes

The Master Plan's P3.5 requires "Workspace/provider spend budgets and a global safe limit, enforced before dispatch and bounded across retries", and asks that the delivery allowance and the compute budget be explained separately.

Before this slice nothing in the app capped paid provider work. The design's exploration of `main` at `96519fc` found three uncapped paths:

- **Public scans** were limited only by `scan_start` (10 per hour per IP, fail-open).
- **Owner rescans** were limited by the paid tier and a 3-per-day rate limit keyed on workspace × IP.
- **AI drafts** had per-request rate limits only.

A runaway loop, a crawler cycling IPs, or one heavy workspace could run up SerpApi, RapidAPI, Google Places and LLM spend without bound.

**Correction: the global safe limit held only from `5971d55`.** As first built and recorded (`90ef323`, `6bdd438`), the claim skipped the budget for every first attempt, while a pending job counted towards the budget only for 24 hours after it was created, and a `queued` job stays claimable forever. Jobs admitted on earlier days and left unclaimed could therefore all be claimed in one window, each unchecked, past the global limit. Production runs no cron reclaim (`CRON_SECRET` is unset), so nothing claims such jobs early. The first record presented the global limit as a hard bound on scan attempts; that was an overstatement. The bound holds only after `5971d55` (review finding C1, below).

**Allowance and budget are different controls.**
- The **delivery allowance** (`workspace_usage`, `export_output_version`) limits what an owner may *ship*: approved deliveries per month. This slice does not touch it.
- A **budget** limits what the app may *spend* producing evidence and drafts, whoever asks: scan attempts, and recorded AI cost, in a rolling 24 hours.

### Decisions (owner, 2026-09-25), as built

| Question | Decision |
|---|---|
| Units | Scans count **scan attempts, retries included**, from a new `scan_attempts` log that the claim statement writes itself (migration `0009`). AI counts the **US$ `action_runs.cost_usd` already recorded** per run. No provider price is invented. |
| At the limit | **Refuse new work before any provider or model call**, with honest localised copy. Admitted work finishes: a first attempt is not re-checked at its claim while its reservation is live, that is, within 24 hours of the job's creation. From `5971d55`, a first attempt whose reservation has expired is metered like a retry. A refused retry stays claimable, so the cron reclaim offers it again on a later tick. |
| Refusal answers | `POST /api/scan/start` → `503 { error: "at_capacity" }`. Rescan → `503 at_capacity` (global) or `429 { error: "workspace_scan_budget_reached" }` (workspace). `POST /api/scan/process` for a refused retry → `503 at_capacity`. Owner draft run and assistant draft → `429 { error: "ai_budget_reached" }`. Create → `201 { runError: "ai_budget_reached" }` (item 4 below). |
| When the check itself fails | An invalid variable or an unreadable count **refuses**; it never lets work through unmetered. It logs `console.error("[budget] check_failed", { entry, reason })`, `reason` being `configuration` or `query`. An over-limit refusal logs exactly `console.warn("[budget] refused", { scope, entry, used, limit })`. Neither carries a name, email or id (`lib/budgets/log.ts`). |
| Configuration | Four environment variables, read by `readBudgetConfig(env)` (`lib/budgets/config.ts`): `BUDGET_SCAN_ATTEMPTS_GLOBAL_24H` (unset = **200**), `BUDGET_SCAN_ATTEMPTS_WORKSPACE_24H` (unset = off), `BUDGET_AI_USD_GLOBAL_24H` (unset = **US$20**), `BUDGET_AI_USD_WORKSPACE_24H` (unset = off). The literal `off` disables one. Zero, negative, non-numeric, padded and empty values throw `budget_configuration_invalid`, naming only the variable. **The two defaults are placeholders for the owner to review, not commercial decisions.** |
| Attempt log | `scan_attempts (id, job_id → audit_jobs ON DELETE CASCADE, workspace_id → workspaces ON DELETE SET NULL, attempted_at)`, two rolling-window indexes, and two `created_at` indexes on `action_runs` for the AI sum. RLS, grants and the `server_application` policy follow 0006. "Used" is one number everywhere: attempts in the last 24 hours plus pending jobs (non-terminal, `attempt_count = 0`, created in the last 24 hours), each pending job holding one reserved attempt. |

### What changed, by task

Full diff `96519fc..90ef323`: **64 files changed, 6,229 insertions, 49 deletions**, across 14 commits. Excluding `docs/`: 62 files, 2,054 insertions, 49 deletions. Task 10 Step 1, `git diff --stat origin/main -- packages neon/migrations/0001_identity.sql … neon/migrations/0008_workspace_internal.sql app/api/cron/dispatch/route.ts lib/repositories/scheduler.ts`, prints nothing.

| Task | Commit(s) | What it did |
|---|---|---|
| Design and plan | `68d7377`, `c393257`, `a117c10` | The design; the plan, written against the code; then honest scanning-page copy and the planning amendments recorded in the spec. |
| 1. Configuration | `8b05abc` | `lib/budgets/config.ts` (`readBudgetConfig`, `BudgetConfigurationError`, the four variables, the two defaults) and its test. `.env.example` documents the four variables as **commented** lines (item 6). |
| 2. Migration `0009` | `a4fa70e` | `neon/migrations/0009_scan_attempts.sql`; the Drizzle `scanAttempts` table and two `action_runs` indexes (`lib/db/schema/business.ts`); the `legacy-final-catalog.json` fixture; the `neon-schema` baselines (36 tables, 422 columns, 162 constraints, 92 indexes, journal 9, 38 Drizzle tables). A new schema test proves the runtime role can log and read attempts, that attempts cascade with their job and outlive their workspace. Nothing writes the table yet. |
| 3. Scan-budget SQL | `18a1b14` | `lib/budgets/log.ts` (the two fixed lines) and `lib/budgets/scan.ts`: one advisory-lock key, one definition of "used", `admitScanJob`, `BUDGETED_CLAIM_SQL` (a data-modifying CTE that claims the job and inserts its `scan_attempts` row together), `claimScanJob`, `ScanBudgetRefusal`. |
| 4. Admission | `4b61855`, `b12db38` | `jobsRepository.insert` runs `admitScanJob` first, on its own transaction client, under the lock, before the job, consent or `scan_started` rows. `insertScanJob` passes a `ScanBudgetRefusal` through as itself. `POST /api/scan/start` answers 503. `enqueueRescan` maps the refusal to `at_capacity` / `workspace_scan_budget_reached` with no audit event and no failure log; the rescan route answers 503 / 429 and creates no schedule. New `neon-scan-admission` suite. `b12db38` makes that suite count consent rows and `scan_started` events too, so "a refusal writes nothing" is proven for all three. |
| 5. Metered claim | `7e23dd7` | The production claim in `lib/scan/execution-store.ts` runs `claimScanJob` in its own short transaction (lock, then the one claim statement), committed before collection starts. A retry over budget is not claimed; the store reports it through `onBudgetRefused`, `runScan` returns `at_capacity`, and `POST /api/scan/process` answers 503. A cron characterisation test pins "skip, retry on a later tick". New `neon-scan-claim-budget` suite, plus one test beyond the plan (concurrent claims of one job write one attempt row, with and without the lock). |
| 6. AI budget, owner runs | `a02994f` | `lib/budgets/ai.ts` (`checkAiBudget`, `AiBudgetRefusal`) and `artifactRepository().aiSpend24h` (both sums in one query; null costs count as zero). `runAgentForAction` checks right after the agent is resolved, before any evidence read, run row or model call, and throws `RunError("ai_budget_reached")`. The run route answers 429; `POST /api/actions` keeps the action and answers `201 { runError }`. New `neon-ai-spend` suite. |
| 7. AI budget, assistant drafts | `fe08cf8` | The assistant's `draft()` checks immediately before `llmComplete` and throws `AiBudgetRefusal`; `POST /api/assistant/run` answers 429. **Targeted improvement:** a draft that fails after the model call is now a terminal `failed` `action_runs` row with its measured `cost_usd`, so the budget sees that spend. Recording is best-effort, and the failed run can never be redeemed into a version. |
| 8. Scan refusal copy | `fa1416e`, `90ef323` | A new app-authored `budget` namespace in `lib/messages/{en,zh-HK,zh-TW}.json` and `lib/budgets/messages.ts`. The scan page, the scanning page's resume and the rescan button each show their own refusal. `90ef323` makes the scanning copy name no button, uses 儲存 (as the rest of `lib/copy.ts` does) instead of 保存, and removes a code comment that promised the cron reclaim continues the scan. |
| 9. AI refusal copy | `2c7328b` | Action detail names `429 ai_budget_reached` before the generic "too many requests" toast; Create shows the limit for `runError: "ai_budget_reached"`; the assistant sheet shows the limit as an answer, not as "The run could not complete". |
| 10. Gates, rollout and this record | *(this commit)* | Gates; an independent re-run of every mutation check; the generated production statement `docs/implementation/owner-platform-v1/rollout/apply-0009.sql` and its rehearsal; a `.gitattributes` line keeping that file LF; this record and the matching `PHASE-3-TEST-RESULTS.md` section. |

### The design as built

The spec's "Amendments made during planning" and the plan's "Where this plan departs from the spec" describe the code as built. Each item, with its reason:

1. **The lock is its own statement.** The spec put the advisory lock inside the claim statement. Under READ COMMITTED a statement's snapshot is taken when the statement starts, so a lock acquired mid-statement would count from a snapshot older than the lock, and two requests could both see the last slot free. Admission and the claim both run `SELECT pg_advisory_xact_lock(hashtextextended('budget:scan-attempts',0))` as the first statement of their transaction. The claim's `UPDATE` and `INSERT INTO scan_attempts` are still one data-modifying CTE, so a claim and its attempt row cannot exist without each other.
2. **The refusal signal is carried host-side.** The vendored `processScan` reports any null claim as `already_claimed` and cannot say why. The store takes an `onBudgetRefused(scope)` option; `runScan` reports `{ status: "at_capacity" }` when the engine says `already_claimed` and that callback fired. `packages/**` is unchanged.
3. **Claim errors stay `claim_failed`.** At the claim, the budget count *is* the claim statement, so a SQL error there cannot be told apart from any other claim failure. It keeps today's behaviour: nothing is claimed, and the route answers 500. An **invalid configuration** at claim time refuses every retry (a global limit of 0 for that statement) and still claims first attempts, which were admitted already. From `5971d55` that exemption covers only first attempts whose reservation is live; an expired one is refused with the retries.
4. **Create gets a 201.** `POST /api/actions` creates the action before running the agent and already reports agent errors as `201 { actionId, runError }`. A budget refusal arrives as `runError: "ai_budget_reached"`, and the Create page shows the AI-limit copy. The action row exists, which is honest: only the draft was refused.
5. **The copy lives in a `budget` namespace.** Action detail, Create and the assistant sheet use `isChinese ? … : …` ternaries, which cannot keep zh-HK and zh-TW apart. The AI copy and the public scan copy therefore live in `lib/messages/*.json` under `budget` (the `applied` / `verified` precedent), and `tests/i18n.test.ts` lists `budget` in `APP_NAMESPACES`. The scanning page's copy is `funnel.scanning.atCapacity` in `lib/copy.ts`; the rescan copy is in the button's inline `COPY`.
6. **The `.env.example` lines are commented out.** An empty value is invalid by design and refuses all work, so four empty assignments copied into `.env.local` would refuse every scan and draft.
7. **Honest copy.** The scanning page says the refused scan "is saved; try resuming it again in a few hours", and promises no automatic continuation, because that depends on the cron reclaim being scheduled in production. The rescan button's global refusal says "Scanning is at capacity right now. Please try again in a few hours." and not the public "Free scans…" text, which would be untrue for a paid owner. **The rescan wording is an assumption awaiting the owner.**
8. **The Task 7 fixture fix.** `neon-assistant-live`'s `request()` now uses a real `app_users` row as the membership's `userId`. The shared `auth()` fixture's `"user-1"` cannot satisfy `action_runs.requested_by` (a uuid FK to `app_users`), so the plan's facts-needed test would have written no row. This is test-only: production takes the user id from `workspace_members.user_id`.
9. **`no_model_output`.** When `llmComplete` returns `null`, the failed run records `error = 'no_model_output'` with a null cost and null tokens, not `invalid_output`. `invalid_output` is kept for a response that came back but did not parse. `RecordAssistantDraftFailureInput.reason` gains the value.
10. **The scanning copy names no button** (`90ef323`). The first wording told people to press "Resume" / 「繼續掃描」, but the button reads "Ask us to resume the scan" / 「請系統繼續這次掃描」.
11. **The log entry is inferred at insert.** `jobsRepository.insert` logs `entry: "rescan"` when the row carries a `workspace_id` and `"scan_start"` otherwise. `POST /api/scan/start` never forwards a workspace, and the rescan path always sets one. This avoids changing the insert's signature and every mock of it.
12. **Two existing integration tests changed.** `neon-execution` "does not reclaim a lease exactly thirty minutes old" froze `now()` by running the store's single `UPDATE` inside the test's own transaction. The claim now opens its own transaction, so the test calls `claimScanJob` on its transaction's client. `neon-assistant-live` "withholds nonempty facts-needed output and performs no artifact persistence" asserted that no `action_runs` row exists. The targeted improvement writes exactly that row, so the test is now "…, creates no artifact, and records the failed run with its cost": one failed run with its cost, still no version and no audit event.

**Known, not changed:**
- `lib/db/database.types.ts` is not regenerated. It already lacks `action_applications`; 0006 and 0008 did not regenerate it either.
- The pending-job count scans `audit_jobs` without a supporting index, because the spec's index list was kept exactly. Review finding M7 below; still not changed.
- The SQL function `claim_audit_job` would claim without the budget, but it has no production caller. From `a10a7d8` a guard test keeps it that way (review finding M4).

### Verification

Full detail is in `PHASE-3-TEST-RESULTS.md`. Summary, one gate at a time at `90ef323`:

| Command | Result |
|---|---|
| `corepack pnpm typecheck` | **passed**: exit 0 |
| `corepack pnpm lint` | **passed**: exit 0, 30 warnings / 0 errors across 18 files, the same counts as `origin/main`. One of the 18, `app/api/scan/process/route.test.ts`, is touched by this branch. Its one warning (`945:34 '_input' is defined but never used`) is identical at `origin/main`, on the same line; the branch appends tests after it. |
| `corepack pnpm test` | **passed on re-run.** First run: exit 1, one failure, `app/api/versions/[versionId]/versions.test.ts` "approves this exact version and reports idempotent on a repeat" (`Test timed out in 5000ms`, then `expected 503 to be 200`). No commit on this branch touches that route or its test. Re-run: exit 0, **329 files / 3,552 tests**, zero failures. `origin/main` measured at 324 / 3,450, so **+5 files, +102 tests**, all in the app suite. |
| `corepack pnpm test:integration` | **passed**: exit 0, **33 files / 346 tests** (232.79s). `origin/main` measured at 30 / 323, so **+3 files, +23 tests**. |
| `corepack pnpm db:verify` | **passed**: exit 0, 0001–0009 applied, replay empty, **36 tables, 422 columns, 162 constraints, 92 indexes**, 8 triggers, 14 functions. `origin/main` measured at 35 / 418 / 159 / 87 / 8 / 14: +1 table, +4 columns, +3 constraints, +5 indexes, exactly Task 2's prediction. |
| `corepack pnpm build` | **blocked on this Windows machine**: exit 1, `Error: Turbopack build failed with 5 errors`, the standing `radix-ui` cascade. Not worked around. Separately, `npx next build --webpack` compiles clean (exit 0, 29/29 static pages). |
| `corepack pnpm e2e`, `corepack pnpm e2e:acceptance` | **not run**: both need a production build, which this machine cannot produce. CI runs both. |

**Mutation checks.** The implementers reported the mutation checks the plan names for Tasks 1–9. Task 10 re-ran every one independently with a scratch script. The script applied each mutation by exact pattern, refusing unless every pattern matched exactly once; ran the named test file(s); wrote the original bytes back; and compared them. **48 runs: 46 plan mutations and 2 valid-query variants. All 48 were killed**, each by the test the plan names, and every file was restored byte-identical. Two of the plan's literal mutations fail for a different reason than the plan expects. Each was therefore run a second time in a form that isolates the behaviour:

- **Task 4, the workspace-scope mutation** (`const scope = "";` in `usedSql`). The admission statement still passes `$1::uuid`, which no longer appears in the SQL, so Postgres fails with an undeterminable parameter type. Admission treats that as an unreadable count and refuses (`check_failed`), so 5 of 6 tests fail on a query error. The valid-query variant (`${workspace} IS NOT NULL AND `, which keeps the parameter but counts every workspace) fails exactly the named test, "refuses a rescan on its workspace limit, not another workspace's or a public scan", and nothing else.
- **Task 6, the `FILTER` mutation** in `aiSpend24h`. Deleting `FILTER (WHERE workspace_id=$1)` leaves `$1` unused, with the same query error (`artifact_operation_failed`, 2 of 3). The valid-query variant (`FILTER (WHERE $1::uuid IS NOT NULL)`) fails the named test, "sums recorded cost in the last 24 hours, globally and for one workspace", on its assertion (`workspaceUsd: 8`, expected 5), as the plan predicted.

Per task (killed / run): Task 1 4/4, Task 2 3/3, Task 3 5/5, Task 4 7/7 (6 + the scope variant), Task 5 8/8, Task 6 7/7 (6 + the filter variant), Task 7 6/6 (the facts-needed deletion run against both the unit and the integration test the plan names), Task 8 4/4, Task 9 4/4. The per-mutation table is in `PHASE-3-TEST-RESULTS.md`.

### Verification checklist

Walked item by item against the code at `90ef323`. Every named test passed in the gate runs above.

| Item | Status | Evidence |
|---|---|---|
| With no variables set, the limits are 200 scan attempts and US$20 per rolling 24 hours, globally, and the per-workspace limits are off | **Holds** | `lib/budgets/config.test.ts` "applies the conservative global defaults and leaves per-workspace limits off" (T1.1 killed). The window is `now() - interval '24 hours'` in `lib/budgets/scan.ts` and in `aiSpend24h`: "admits under the limit and refuses at it, counting attempts in the last 24 hours only" (T4.4 killed) and "sums recorded cost in the last 24 hours…" (T6.3 killed). |
| `off` disables a limit, and every other invalid value throws `budget_configuration_invalid` naming the variable | **Holds** | "turns any limit off with the literal off"; 20 cases of "rejects `<variable>=<value>` with a coded error that names only the variable" (zero, `0.00`, negative, a decimal for an integer, words, `OFF`, padded, empty, `1e3`, an unsafe integer, `$20`, `Infinity`, `1.`); "names exactly the four documented variables". T1.2–T1.4 killed. |
| Migration `0009` adds `scan_attempts`, two indexes on it and two on `action_runs`, with RLS, grants and the server-only policy; `db:verify` reports 36 / 422 / 162 / 92, journal 9 | **Holds** | `db:verify` above. `neon-schema` "applies all final business objects…" fails on "all final indexes and predicates" (T2.1), "business tables and RLS" (T2.2) and "constraints and deletion semantics" (T2.3). "lets the runtime role log scan attempts, cascading with the job and outliving the workspace". The rollout rehearsal (Runbook) confirms the table, both indexes, the policy and runtime SELECT/INSERT on production-shaped roles. |
| Every claim writes exactly one `scan_attempts` row carrying the job's workspace, and a job that is not claimed gets none | **Holds** | `neon-scan-claim-budget` "writes exactly one attempt row per claim, carrying the job's workspace", "concurrent claims of one job write exactly one attempt row, with the lock and without it", "writes no attempt row for a job it did not claim". T5.4 (delete the `attempt` CTE) fails four tests. |
| A first attempt is claimed even when the budget is spent | **Holds, narrowed by `5971d55`** | "still runs an admitted first attempt when the budget is already spent" (T5.2 killed); "claims first attempts but refuses retries when the configuration is invalid". As recorded at `90ef323` this held for *every* first attempt, which was review finding C1. It now holds only while the job's reservation is live; "meters a first attempt whose reservation has expired, like a retry" and "claims an expired first attempt while the budget has room" cover the rest. |
| An over-limit retry stays unclaimed, keeps its status and attempt count, remains claimable for the cron, and reports `at_capacity` → `503 { error: "at_capacity" }` | **Holds** | "leaves an over-limit retry unclaimed and reports at_capacity" (integration); `lib/scan/execution-store.test.ts` "tells the host that a retry was refused on budget, and returns no job" (T5.6); `lib/scan/run.test.ts` "reports at_capacity when the store refused the claim on budget, and completes nothing" and the process route's "answers 503 at_capacity when the store refused a retry on budget" (T5.7 fails both). |
| Pending jobs count as reserved attempts, at admission and at the claim, and pending jobs older than 24 hours do not | **Holds** | `neon-scan-admission` "counts pending jobs as reserved attempts, but not pending jobs older than the window" (T4.2, T4.4 killed); `neon-scan-claim-budget` "counts pending first attempts against a retry" (T5.3 killed); unit "one definition of used counts the same attempts and pending jobs at admission and at the claim" (T3.5 killed). |
| Two admissions racing for the last slot admit exactly one, and a burst of eight admits exactly the free slots | **Holds** | "admits exactly one of two requests racing for the last slot" and "lets a burst of eight admit exactly the three free slots". Deleting the lock (T4.1) fails both. |
| Two retries racing for the last slot are serialized by the lock | **Holds** | "serializes retries racing for the last slot". Deleting the claim's lock (T5.1) fails exactly that test. |
| The workspace limit applies only to that workspace; a public scan and another workspace are unaffected | **Holds** | Admission: "refuses a rescan on its workspace limit, not another workspace's or a public scan" (the T4.3 variant fails it alone; see the mutation note). Claim: "applies the workspace limit only to that workspace's retries" (T5.5 killed). |
| `POST /api/scan/start` answers `503 at_capacity`; the rescan route answers `503 at_capacity` or `429 workspace_scan_budget_reached`; the cron reclaim treats a 503 as skip-and-retry | **Holds** | `app/api/scan/start/route.test.ts` "spend budget answers 503 at_capacity, forwards nothing and logs no persistence failure, when admission refuses"; the rescan route's "maps the at_capacity refusal to 503…" and "maps the workspace_scan_budget_reached refusal to 429…, and creates no schedule"; `lib/workspace/rescan.test.ts`, both refusal mappings; the cron route's "treats 503 at_capacity from scan/process as skip-and-retry: nothing logged, the job offered again next tick" (T5.8 killed). *Scope:* the cron test proves the route logs nothing and offers the job again. Whether a later tick happens in production depends on `CRON_SECRET` (see below). |
| The AI sum reads only `action_runs.cost_usd` in the window, globally and per workspace | **Holds** | `neon-ai-spend` "sums recorded cost in the last 24 hours, globally and for one workspace" (the T6.2 variant and T6.3 killed), "reads zero, not nothing, when no run is recorded", "fails rather than answering when the read cannot run". |
| `runAgentForAction` and the assistant's `draft()` refuse before `llmComplete` and answer `429 ai_budget_reached`; Create gets `runError: "ai_budget_reached"` with its action kept | **Holds** | `lib/workspace/runs.test.ts` "refuses before any run row or model call once the global spend reaches the limit" (T6.1 killed: `queue` was called), "refuses on the workspace limit when one is set, and runs below it", "refuses when the recorded spend cannot be read"; the run route's "answers 429 ai_budget_reached before any run row or model call" (T6.6); `lib/assistant/live.test.ts` "refuses a draft before the model when the recorded spend reaches the limit" (T7.1); the assistant route's "answers 429 ai_budget_reached when the live draft was refused, and records no run event" (T7.5); `app/api/actions/route.test.ts` "keeps the created action and reports an AI budget refusal as its run error". |
| A failed assistant draft records a `failed` run with its measured cost, and can never be redeemed into a version | **Holds** | Unit: "records a parse failure as a failed run with its measured cost, then degrades" (T7.2), "records a request for missing facts as a failed run too" (T7.3), "records a draft whose model returned nothing as no_model_output with a null cost", "still answers when the failed run cannot be recorded" (T7.4). Integration: "withholds nonempty facts-needed output, creates no artifact, and records the failed run with its cost" (T7.3i), "records a invalid_output draft…" and "records a facts_needed draft as exactly one failed run that aiSpend24h counts", "refuses the next draft before the model once recorded failed-draft spend reaches the workspace limit". Not redeemable: `assistantDraft` reads only `state = 'succeeded'`, and the facts-needed integration test asserts no version and no artifact. |
| An error in any budget check refuses the work and logs `[budget] check_failed`; an over-limit refusal logs exactly `[budget] refused { scope, entry, used, limit }`, with no names, emails or ids | **Holds** | The three "refuses, with the check_failed line, when …" cases in each of `scan.test.ts` (T3.3) and `ai.test.ts` (T6.5); "refuses an invalid configuration before touching the database" and "…without reading spend"; `neon-scan-admission` "refuses, and writes nothing, when the budget count cannot be read". Exact line: `toHaveBeenCalledWith("[budget] refused", { scope, entry, used, limit })` in `scan.test.ts` and `ai.test.ts` (T3.2, T6.4). No names: by construction, `logBudgetRefusal` and `logBudgetCheckFailed` accept only the scope, entry, reason and two numbers. No test scans the log output for identifiers. |
| Every refusal message exists in en, zh-HK and zh-TW, and each surface shows it | **Holds** | Key parity: `tests/i18n.test.ts` "share one key set across en, zh-HK and zh-TW" (T8.1). Per surface and locale: `messages.test.ts` (the scan-page copy; T8.2, T9.4), `scanning-page.test.tsx` "says in %s that a refused resume is saved and can be resumed later" (T8.3), `phase6-ui.test.tsx` "names the workspace limit and global capacity separately in %s" (T8.4), `action-detail-client.test.tsx` (T9.1), `create-view.test.tsx` (T9.2), `assistant-sheet.test.tsx` (T9.3). *Not rendered:* the public scan page (`components/scan-page.tsx`) is covered through `scanStartRefusal` in `messages.test.ts`, not by a component test. |
| `packages/**`, migrations `0001`–`0008`, the cron route and the scheduler repository are unchanged | **Held at `90ef323`; the scheduler repository changed in `b600e77`** | Task 10 Step 1: the `git diff --stat origin/main -- …` printed nothing. Package test counts are identical to P3.2c's. Review finding M1 then changed `lib/repositories/scheduler.ts` on purpose: the reclaim query now has an `ORDER BY`. `packages/**`, migrations `0001`–`0009` and the cron route are still unchanged at the final head. |
| Every mutation check failed its named test | **Holds** | 48 of 48 killed in the Task 10 re-run. For the Task 4 scope and Task 6 filter mutations, the literal form fails on a query error and the valid-query variant fails the named test on its assertion. |

### Runbook (owner actions)

The statement is [`rollout/apply-0009.sql`](rollout/apply-0009.sql). It was generated from the migration files on disk, not typed. It is one `DO $apply$ … $apply$;` block that
...
- refuses unless `neon_migrations.journal` is **exactly** ordinals 1–8 with the names and sha256 checksums of `neon/migrations/0001…0008`, as `loadMigrations()` computes them;
- `EXECUTE`s the exact text of `0009_scan_attempts.sql` inside `$m0009$` (0009 contains no `$` at all);
- inserts journal row `(9, '0009_scan_attempts.sql', '3c35ab9965be1d4821e885a2e43676c743e62f25435e4d59bd46d2c34a3d1768')`.

Every checksum was re-derived from the file bytes after generation and matched, and the embedded 0009 text is byte-identical to the file. `.gitattributes` now keeps `rollout/*.sql` LF, as it does for `neon/migrations/*.sql`, so the embedded text stays byte-identical on a Windows checkout.

**Rehearsal (2026-09-25, disposable `postgres:16`, server 16.15, run twice with the same results).** The roles matched production: `neondb_owner` LOGIN CREATEROLE, owning database `neondb`; `neondb_owner` created `smeassistant_migrator` NOLOGIN and `sme_app_runtime` NOLOGIN, and granted the migrator CREATE on the database and on schema `public`. 0001–0008 were applied as the migrator through the repository's `applyMigrations` with the first eight migrations. The block was then sent as one query, as `neondb_owner`:

| # | Check | Result |
|---|---|---|
| 5 | Before `GRANT smeassistant_migrator TO neondb_owner WITH SET TRUE` | **refused**: `42501 permission denied to set role "smeassistant_migrator"`. Journal still 8 rows, no `scan_attempts`, and the catalog and journal snapshot unchanged. |
| — | The grant, run as `neondb_owner` | succeeded (`set_option = true`, grantor `neondb_owner`) |
| 1 | First run | **applied**, with notices `policy "server_application" for relation "public.scan_attempts" does not exist, skipping` and `apply-0009: applied 0009_scan_attempts.sql and recorded journal row 9`. Journal rows 1–9 with the expected checksums. |
| 2 | `applyMigrations` with all nine | returned `[]`: nothing pending, so the runner accepts the journal's checksums |
| 3 | Ownership and runtime access | `scan_attempts`, its primary key, both its indexes and both new `action_runs` indexes are owned by `smeassistant_migrator`. RLS is on, with policy `server_application`. `sme_app_runtime` has SELECT and INSERT, and under `SET ROLE sme_app_runtime` inserted and read an attempt row (rolled back). |
| 4 | Second run | **refused**: `P0001 apply-0009 refused: neon_migrations.journal is not exactly 0001-0008 with the expected checksums (it has 9 rows)`. Catalog and journal snapshot unchanged. |

The container was removed afterwards. The generator and the rehearsal script were scratch files and are not committed.

**Owner steps:**

1. **Apply `apply-0009.sql` on a Neon test branch of production, then on production, before deploying.** Run it in the Neon SQL Editor, logged in as `neondb_owner`. The new claim writes to `scan_attempts`, so **every scan fails without the table**. `neon:readiness` reports `not_ready` / `schema` until the journal holds 0009 with the repository's checksum and `scan_attempts` exists, because it compares the journal with the files on disk and checks every Drizzle table.
2. **Deploy.** The defaults apply immediately: 200 scan attempts and US$20 of recorded AI cost per rolling 24 hours, globally, and no per-workspace limit.
3. **Optionally set, or turn `off`, the four variables in Vercel.** An empty or invalid value refuses the work it guards and logs `[budget] check_failed` with `reason: "configuration"`.
4. **Check with a read-only query that attempts are logged** as scans run: `SELECT count(*), max(attempted_at) FROM scan_attempts WHERE attempted_at > now() - interval '24 hours';`. Watch the runtime logs for `[budget] refused` and `[budget] check_failed`.
5. **Expect the scan budget to undercount for the first 24 hours after the deploy.** `scan_attempts` starts empty. Attempts made before 0009, and claims made by the old deployment between applying 0009 and deploying, are not in it, so the global limit binds later than it would at steady state during that first day. Pending jobs are counted from `audit_jobs` and are not affected. (Review finding M6.)

### What this slice does NOT prove

1. **That anything works hosted.** Nothing is hosted-verified until `0009` is applied and the app is deployed. The rehearsal ran on Docker Postgres 16 with production-shaped roles, not on Neon.
2. **AI runs with no recorded cost count as zero.** `computeCostUsd` returns null when the gateway omits token usage, and `aiSpend24h` sums with `coalesce`.
3. **Report summaries (`lib/llm-summary.ts`) are not metered.** They persist no cost and may have no workspace. They are cached once per report and locale, so they are bounded by scans.
4. **Provider fallbacks inside one attempt are not counted separately** (for example the SerpApi fallback key). The unit is the attempt.
5. **The AI pre-check can overshoot by one run per concurrent request.** A run that starts under the limit may finish above it by its own cost.
6. **A `social_post` action with no approved asset is refused while the AI budget is spent**, although that run would not call the model, because the pre-check sits before the asset check (plan departure 6). **No committed test covers this case.**
7. **Automatic continuation after `at_capacity` depends on the cron reclaim.** `app/api/cron/dispatch` re-offers claimable jobs only if it is scheduled with `CRON_SECRET` set in production, which P3.1 recorded as not done. The copy therefore promises only that the scan is saved and can be resumed.
8. **The Chinese copy has not been reviewed by a native speaker.** It follows the repository register rules: 香港書面中文 for zh-HK, 台灣用語 for zh-TW, and 工作台 as the workspace term. The rescan at-capacity wording is an assumption awaiting the owner.
9. **The defaults are unreviewed placeholders.** 200 attempts and US$20 per 24 hours are not commercial decisions.
10. **That the global limit was a hard bound as first recorded.** It was not (C1 below); it is from `5971d55`. The residuals the review accepted (M2, M5, M6, M7) are listed after the review table.

### Whole-branch review and its fixes

The sections above describe the Task 10 candidate `90ef323` and its record `6bdd438`. A final review of the whole branch against `origin/main` then found one Critical finding and seven Minor ones. Each code fix was written test-first: the new test was run and seen to fail for the reason the finding names, the fix made it pass, and then the fix was reverted to confirm the new test fails again (the mutation check), and restored.

| Finding | Resolution |
|---|---|
| **C1 (critical).** An expired reservation claimed unchecked. The claim exempted any first attempt (`target_attempts = 0`), but a pending job counts towards the budget only while `created_at > now() - interval '24 hours'`, and a `queued` job stays claimable forever. Admitted-but-unclaimed jobs could be accumulated over several days and then all claimed in one window, each an unchecked "first attempt", past the global limit. Production runs no cron reclaim (`CRON_SECRET` unset), so nothing claims them early. | **Fixed in `5971d55`.** The exemption is now `(target_attempts = 0 AND target_created_at > now() - interval '24 hours') OR (<within budget>)`, with `created_at AS target_created_at` in the `target` CTE. The window is not typed twice: `lib/budgets/scan.ts` derives both the pending count (`PENDING_JOB_CONDITION_SQL`, byte-identical to before) and the claim's exemption from one `reservationLiveSql()` over the existing `WINDOW_SQL`, so a job stops being exempt at exactly the moment it stops being counted. A refused expired first attempt reports `at_capacity` through `onBudgetRefused` and logs the same line as a retry, with `entry: "retry_claim"`: that entry names the claim-time gate, which now meters every claim that is not a live first attempt (the `BudgetEntry` comment says so). New `neon-scan-claim-budget` tests: "meters a first attempt whose reservation has expired, like a retry" (created 25 hours ago, global budget spent: not claimed, `at_capacity`, the fixed log line, still `queued` with `attempt_count = 0`, no attempt row) and "claims an expired first attempt while the budget has room" (claimed, one row). "still runs an admitted first attempt when the budget is already spent" still passes. A unit assertion in `scan.test.ts` pins the exemption's SQL. **RED:** the expired job was claimed (`expected { … } to be null`). **Mutation:** restoring the unconditional `(target_attempts = 0` fails "meters a first attempt whose reservation has expired, like a retry", 1 of 11. |
| **M1.** The cron reclaim (`schedulerRepository().claimableJobIds`) selected `LIMIT 20` with no `ORDER BY`. A refused retry stays claimable, so each tick could keep offering the same refusals and starve admitted jobs. | **Fixed in `b600e77`.** `ORDER BY attempt_count, created_at, id`: first attempts first, then the oldest, with `id` as a stable tie-break. New `neon-cron-dispatch` test "offers first attempts before retries, oldest first, so refused retries cannot fill the batch": 21 claimable retries plus one queued first attempt, the retries inserted in exactly the reverse of the expected order, so heap order without the `ORDER BY` can never match. It asserts the queued job is in the batch and the batch's exact order. **RED:** the queued job was missing from the 20. **Mutation:** removing the `ORDER BY` fails the test, 1 of 6, on each of three consecutive runs. |
| **M2.** A rescan refused on budget still consumes one of the owner's 3 daily rescans. | **Accepted, not changed.** See the residuals below. |
| **M3.** Failed assistant drafts store raw reason codes (`facts_needed`, `invalid_output`, `no_model_output`) in `action_runs.error`. The owner's workflow tab printed them raw ("faq_jsonld · Failed · facts_needed"), and the P3.4 value report's "Task runs failed" began counting them. | **Fixed in `838888e`.** `runErrorLabel` (`lib/assistant/draft-failure.ts`) maps the three codes to a new app-authored `draftFailure` namespace in `lib/messages/{en,zh-HK,zh-TW}.json` (the `budget` precedent; `tests/i18n.test.ts` lists it in `APP_NAMESPACES`): en "Needed more facts" / "Draft could not be read" / "AI drafting unavailable"; zh-HK 需要更多資料 / 未能讀取草稿 / AI 草稿生成暫時未能使用; zh-TW 需要更多資訊 / 無法讀取草稿 / AI 草稿生成目前無法使用. Both places the workflow tab shows `error` use it; every other string (a task run's friendly error) passes through unchanged. The value report identifies assistant runs by `input->>'source' = 'assistant'`, which is what `recordAssistantDraft` and `recordAssistantDraftFailure` write (`lib/repositories/artifacts.ts`) and what `assistantDraft` already filters on; task runs have no `source` key. Both sides of "failed of runs" now exclude them (`TASK_RUN` in `scripts/report/value-queries.ts`). New tests: `action-detail-client.test.tsx` "labels every reason code in %s and leaves other errors as they are" (3 locales); `neon-value-report` "leaves assistant drafts, failed or succeeded, out of the task-run counts" (rolled back). **RED:** the labels were missing in all three locales; the report read `{ runs: 4, failed: 2 }` against `{ runs: 2, failed: 1 }`. **Mutations:** removing `runErrorLabel` from the component fails all 3 locale cases; removing the filter from `failed` only gives `failed: 2`, from `runs` only gives `runs: 4`. |
| **M4.** `workflowRepository().claimAuditJob` wraps `claim_audit_job`, which claims without writing a `scan_attempts` row. Only an integration test calls it. | **Guarded in `a10a7d8`.** The method and the SQL function stay: the integration test uses them and migration `0004` is immutable. `tests/scan-claim-single-path.test.ts`, modelled on `scan-events-single-writer`, walks non-test sources under `lib/`, `app/` and `scripts/` and fails if any file references `claimAuditJob` or invokes `claim_audit_job(`, apart from the wrapper's own definition in `lib/repositories/workflow.ts`, which now carries a comment saying so. Comments and the catalog's name list are mentions, not invocations, and are not flagged; self-tests pin both sides. **Mutation:** a planted `repo.claimAuditJob("x")` in `lib/scan/claimable.ts` failed it, and so did a planted `"SELECT * FROM public.claim_audit_job($1)"` string; each was removed. |
| **M5, M6, M7.** A rare deadlock with a workspace delete; the budget starting from zero at rollout; the pending count's sequential scan. | **Accepted, not changed.** See the residuals below. M6 adds runbook step 5. |

**Branch as offered:** `p35a-spend-budgets` at `a10a7d8` plus this documentation commit: 20 commits on `96519fc`. `git diff --stat origin/main -- packages neon/migrations app/api/cron/dispatch/route.ts docs/implementation/owner-platform-v1/rollout` lists only the new `0009_scan_attempts.sql` and `apply-0009.sql`, and `git diff 6bdd438 -- neon docs/implementation/owner-platform-v1/rollout` is empty: no review fix touched either.

**Gates at `a10a7d8`**, run sequentially:

| Gate | Result |
|---|---|
| `corepack pnpm typecheck` | passed, exit 0 |
| `corepack pnpm lint` | passed, 30 warnings / 0 errors |
| `corepack pnpm test` | passed, exit 0. **330 files / 3,560 tests** (app 279 / 2,973, safe-media 1 / 62, region 3 / 23, scoring 16 / 183, contracts 3 / 20, scan-engine 28 / 299). Against `90ef323`'s 329 / 3,552: +1 file (`tests/scan-claim-single-path.test.ts`, 4 tests), +8 tests (that file, 3 in `action-detail-client.test.tsx`, 1 in `lib/budgets/scan.test.ts`). |
| `corepack pnpm test:integration` | passed, exit 0. **33 files / 350 tests** (226.29s). Against `90ef323`'s 33 / 346: +4 tests, no new file: 2 in `neon-scan-claim-budget` (now 11), 1 in `neon-cron-dispatch` (now 6), 1 in `neon-value-report` (now 13). |
| `build`, `db:verify` | not re-run. No review fix touches `package.json`, the lockfile, `components/ui`, or `neon/migrations`. |

### Known residuals, not fixed

- **M2: a rescan refused on budget still consumes one of the owner's 3 daily rescans.** `app/api/workspaces/[workspaceId]/rescan/route.ts` runs `enforceRateLimit({ scope: "rescan", … })` before `enqueueRescan`, so the rate-limit bucket is spent before admission refuses. An owner who hits the scan budget three times in a day is then told they are rate limited. Fixing it means reordering or refunding a fail-closed rate limit, which this slice does not take on.
