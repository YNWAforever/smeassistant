...
**Branch** `p35a-spend-budgets` · **HEAD** `90ef323` (the last code commit) plus this documentation commit · Base: `main` at `96519fc` (PR #20, merged, P3.2c; `origin/main` still points there). Node `v24.18.0`, pnpm `9.12.0` via corepack, Windows 11, Docker Server `29.7.2`.
...
| At the limit | **Refuse new work before any provider or model call**, with honest localised copy. Admitted work finishes: a first attempt is never re-checked at its claim. A refused retry stays claimable, so the cron reclaim offers it again on a later tick. |
...
3. **Claim errors stay `claim_failed`.** At the claim, the budget count *is* the claim statement, so a SQL error there cannot be told apart from any other claim failure. It keeps today's behaviour: nothing is claimed, and the route answers 500. An **invalid configuration** at claim time refuses every retry (a global limit of 0 for that statement) and still claims first attempts, which were admitted already.
...
- The pending-job count scans `audit_jobs` without a supporting index, because the spec's index list was kept exactly.
- The SQL function `claim_audit_job` would claim without the budget, but it has no production caller.
...
| A first attempt is claimed even when the budget is spent | **Holds** | "still runs an admitted first attempt when the budget is already spent" (T5.2 killed); "claims first attempts but refuses retries when the configuration is invalid". |
...
| `packages/**`, migrations `0001`–`0008`, the cron route and the scheduler repository are unchanged | **Holds** | Task 10 Step 1: the `git diff --stat origin/main -- …` prints nothing. Package test counts are identical to P3.2c's. |
