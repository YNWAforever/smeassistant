...
**Mutation checks.** Task 10 ran 23 mutations, one at a time. Each was restored before the next. 22 were killed by a named test and 1 survived: V9, the scans line's `is_demo` exclusion (see the checklist). The per-mutation table is in `PHASE-3-TEST-RESULTS.md`. It covers:
...
| Every exclusion was mutation-checked and each mutation failed a named test | **Not fully true** | The five mutations the plan names for Task 8 were re-run in Task 10 and all were killed (V1–V5). So were V6–V8. **V9 survived:** removing `is_demo` from the scans line (`AND NOT coalesce(w.is_demo OR w.is_internal, false)` in `scripts/report/value-queries.ts`) fails no test, because the seed attaches no scan to the demo workspace. Recorded as an open gap. Closing it needs one seeded demo-attached scan, and every scan and reconciliation expectation re-derived. Not done in Task 10, which changes documents only. |
...
- **The scans line's demo exclusion is untested** (mutation V9 above). Found in Task 10, not fixed.
