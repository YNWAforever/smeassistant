...
Repository build settings: Next.js, root `/`, Node 22.x (`.nvmrc`), pnpm 9.12.0; install `pnpm install --frozen-lockfile`, build `pnpm build`. Record actual deployed runtime separately. Keep the existing scheduler; do not create a competing cron -- `app/api/cron/dispatch` is not one (see the note at item 6 above). An engine-completed job is not proof that workspace post-processing completed.
