...
**Task 16 local gates and independent review: approved. Task 17: prepared locally, awaiting independent review. Hosted actions: NOT READY / NOT RUN.** Target project, region, branch, database, stable origin, accounts and operators remain NOT CHOSEN. The authoritative active procedure is [NEON-CUTOVER.md](NEON-CUTOVER.md); record outcomes in [LAUNCH-REPORT.md](LAUNCH-REPORT.md).

1. Complete the release worksheet and source/configuration map in NEON-CUTOVER. Runtime source is `853bdee87031cd1d1b2282969ab1b238c498d147`; full-gate evidence includes the protected unstaged owner-shell delta, not the commit alone. Record the eventual exact candidate SHA and review any runtime difference. Preparation commits do not prove remote CI.
...
6. Select Vercel execution. Verify [runner and completion parity](NEON-RUNNER-COMPATIBILITY.md), same job database and one existing scheduler owner. Current code blocks external scheduled/cloudflare dispatch. An unchanged Supabase worker blocks promotion for that mode. Do not create a new cron or move the legacy domain.
