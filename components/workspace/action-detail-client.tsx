      ...
      <Tabs defaultValue="draft" className="action-detail-tabs">
        ...
        <TabsContent value="evidence">
          <div className="evidence-detail-grid">
            ...
            {measurements.length > 0 && <SectionCard><p className="eyebrow">{isChinese ? "量度" : "Measurement"}</p><h2>{isChinese ? "行動前後" : "Before and after"}</h2><dl className="trust-dl">{measurements.map((m) => <div key={m.id}><dt>{metricLabel(m.metric_key, locale)} · <FactType type={m.fact_type} />{m.fact_type === "Attributed" ? <span className="limitation-note"> · {basisLabel(m.attribution_basis, locale)}</span> : null}</dt><dd>{m.before_value ?? "—"} → {m.after_value ?? "—"} ({signed(Number(m.delta), 1)})</dd></div>)}</dl></SectionCard>}
          </div>
        </TabsContent>
