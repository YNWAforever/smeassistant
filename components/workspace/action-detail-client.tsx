...
export function ActionDetailClient({ locale, workspaceSlug, workspaceId, timezone, role, inScope, location, detail, auditRows, locations, approvedAssets }: ActionDetailClientProps) {
  ...
  const provenance = [
    ...
    {
      label: isChinese ? "Agent 輸入" : "Agent input",
      state: neededKeys.length ? "pending" : "complete",
      detail: neededKeys.length
        ? (isChinese ? `缺 ${neededKeys.length} 項資料` : `${neededKeys.length} inputs missing`)
        : reviewEvidence
          // Name the source, so "ready" is never mistaken for "the owner typed it".
          ? `${scanInputCopy.fromScan} · ${reviewEvidence.available}`
          : (isChinese ? "資料齊備" : "Inputs ready"),
    },
  ...
  /**
   * The optional owner addition next to the scanned reviews. Deliberately not
  ...
  async function addOwnerReviewNote() {
    ...
    if (!value) return
    setBusy("inputs")
    const patched = await updateAction(action.id, { provided_inputs: { reviews_without_response: value } })
    setBusy(null)
    if (!patched.ok) return failureToast(patched)
    toast.message(isChinese ? "已記錄店主提供的評論；正在重新生成。" : "Owner-supplied review recorded; generating again.")
    await generate({ reviews_without_response: value })
  }
              ...
              {reviewEvidence && (
                <div className="brand-check-panel">
                  <div className="brand-check-head">
                    <FactType type={reviewEvidence.factType} />
                    <div>
                      <strong>{scanInputCopy.heading}</strong>
                      <span>{action.evidence.source} · {formatDateTime(reviewEvidence.observedAt, locale, timezone)}</span>
                    </div>
                    ...
                    {reviewEvidence.reviews.map((review, index) => (
                      <div key={`${review.time ?? "unknown"}-${index}`}>
                        ...
                        <div>
                          ...
                          <small>{review.time ? formatDateTime(review.time, locale, timezone) : ""}</small>
                        </div>
                      </div>
                    ))}
                  </div>
                  <p className="limitation-note">
                    ...
                    {scanInputCopy.limitation
                      .replace("{inspected}", String(reviewEvidence.inspected))
                      .replace("{unanswered}", String(reviewEvidence.available))}
                    {reviewEvidence.populationCount != null
                      ? scanInputCopy.population.replace("{total}", String(reviewEvidence.populationCount))
                      : ""}
                  </p>
                  {canEdit && (
                    <details>
                      <summary>{scanInputCopy.addOwn}</summary>
                      <div className="field-stack">
                        <Textarea
                          id="input-owner-review"
                          rows={3}
                          value={inputValues.reviews_without_response ?? ""}
                          onChange={(event) => setInputValues((prev) => ({ ...prev, reviews_without_response: event.target.value }))}
                          disabled={!canEdit}
                        />
                        <small>{scanInputCopy.addOwnNote}</small>
                        <div className="draft-editor-actions">
                          <Button
                            variant="outline"
                            onClick={() => void addOwnerReviewNote()}
                            disabled={!canEdit || !(inputValues.reviews_without_response ?? "").trim()}
                          >
                            {busy === "inputs" || busy === "run" ? <LoaderCircle className="animate-spin" /> : <WandSparkles />} {scanInputCopy.addOwnSubmit}
                          </Button>
                        </div>
                      </div>
                    </details>
        ...
        <TabsContent value="workflow">
          <div className="workflow-state-layout">
            <SectionCard><div className="section-card-heading"><div><p className="eyebrow">Agent run</p><h2>{isChinese ? "可復原執行狀態" : "Recoverable run state"}</h2></div><Badge variant="outline">{isChinese ? "獨立狀態機" : "Separate state machine"}</Badge></div>
              <div className={`run-state-box run-${workflowState === "succeeded" ? "ready" : workflowState === "running" || workflowState === "queued" ? workflowState : workflowState ? "failed" : "ready"}`}>{!workflowState || workflowState === "succeeded" ? <Sparkles /> : workflowState === "running" ? <LoaderCircle className="animate-spin" /> : workflowState === "queued" ? <FileClock /> : <CircleAlert />}<div><strong>{workflowLabel}</strong><span>{workflowState === "timed_out" ? (isChinese ? "上一次執行沒有完成並已釋放；現有草稿已保留，可再按生成重試。" : "The previous run did not finish and has been released; your existing draft is preserved — press Generate to try again.") : workflowState === "failed" ? (isChinese ? "現有草稿已保留；失敗不扣除用量。" : "Existing draft preserved; failures do not consume usage.") : (isChinese ? "生成及修改不扣除客戶用量。" : "Generation and revision do not consume customer usage.")}{latestRun?.error ? ` · ${latestRun.error}` : ""}</span></div></div>
