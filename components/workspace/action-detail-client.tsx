...
/**
 * Who wrote this version. Three-way rather than two, because "the operator
 * drafted it" and "an agent run produced it" are different provenance and both
 * differ from a hand edit -- and a version carries an approval, so the label is
 * an accountability claim, not decoration.
 */
function originLabel(origin: VersionRow["origin"], isChinese: boolean): string {
  if (origin === "agent_run") return isChinese ? "由 Agent 生成" : "Generated by agent"
  if (origin === "assistant") return isChinese ? "由助理生成" : "Drafted by the operator"
...
export function ActionDetailClient({ locale, workspaceSlug, workspaceId, timezone, role, inScope, location, detail, auditRows, locations, approvedAssets }: ActionDetailClientProps) {
  ...
  // badge -- that conflation is what let a constant "1 reminder" pass for a
  // real count.
  const guardrailBadge = guardrailFlags.length > 0
    ? (isChinese ? `${guardrailFlags.length} 項違規` : `${guardrailFlags.length} flagged`)
      ...
      <Tabs defaultValue="draft" className="action-detail-tabs">
          ...
          <div className="draft-layout">
            <SectionCard className="draft-editor-card">
              ...
              {/* The agents compute guardrail violations and artifacts.ts
                  persists them into meta.warnings, but nothing ever read them
                  back: this panel showed a hard-coded "1 reminder" on every
                  draft, so a prohibited brand term or a compensation promise
        ...
        <TabsContent value="evidence">
          <div className="evidence-detail-grid">
            ...
            {measurements.length > 0 && <SectionCard><p className="eyebrow">{isChinese ? "量度" : "Measurement"}</p><h2>{isChinese ? "行動前後" : "Before and after"}</h2><dl className="trust-dl">{measurements.map((m) => <div key={m.id}><dt>{metricLabel(m.metric_key, locale)} · <FactType type={m.fact_type} />{m.fact_type === "Attributed" ? <span> · {basisLabel(m.attribution_basis, locale)}</span> : null}</dt><dd>{m.before_value ?? "—"} → {m.after_value ?? "—"} ({signed(Number(m.delta), 1)})</dd></div>)}</dl></SectionCard>}
          </div>
        </TabsContent>
