...
export function AssistantRunStatus({ state, isChinese }: { state: "idle" | "running" | "failed"; isChinese: boolean }) {
  ...
  return (
    <div className={`assistant-run-status is-${state}`} role={state === "failed" ? "alert" : "status"}>
      ...
      <div>
        <strong>{state === "running" ? (isChinese ? "正在讀取固定示範證據" : "Reading the fixed demo evidence") : (isChinese ? "暫時未能完成" : "The run could not complete")}</strong>
